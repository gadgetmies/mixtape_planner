...
const tracks = `It Ain't Bangin'	dope	4A	6
...
const normalize = (newMin, newMax) => values => {
...
const graphWalk = async ({tracks, intros, targetLength, penalties}) => {
  const minAndMaxValuesForPenalties = Object.fromEntries(Object.keys(penalties)
    .map(key => {
      const values = R.pluck(key, tracks)
...
const saw = riseTime => n => {
  const pos = n / riseTime
  return (pos - Math.floor(pos))
}
const absoluteSine = n => Math.abs(Math.sin(n / 2))
const normalizedSine = n => (Math.sin(n) + 1) / 2

const energyFunction = saw(5)
const popularityFunction = normalizedSine
...
graphWalk({
  ...
  .then(paths => {
    ...
    const indexRange = R.range(0, targetLength)
    const energyRange = limits(R.pluck('energy', tracks))

    console.log('tracklist')
    console.log(bestPath.map(({artist, title, keyNumber, isMinor}) => `${artist} - ${title} (${getKeyString(keyNumber, isMinor)})`).join('\n'))
    console.log('energy')
    console.log(normalize(energyRange.min, energyRange.max)(indexRange.map((_, i) => energyFunction(i))).join(separator))
    console.log(bestPath.map(R.prop('energy')).join((separator)))
    console.log('popularity')
    console.log(indexRange.map((_, i) => popularityFunction(i)).join(separator))
